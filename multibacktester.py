...
def backtest(configs):
    ...
    default_config = {"PAIR": config.PAIR, "PERIOD": config.PERIOD, "TRADE_AMOUNT": config.TRADE_AMOUNT, "TAKE_PROFIT": config.TAKE_PROFIT, "STOP_LOSS": config.STOP_LOSS, "POSITION_STRUCTURE": config.POSITION_STRUCTURE, "KLINE_TO_USE_IN_PROD": config.KLINE_TO_USE_IN_PROD, "KLINE_INTERVAL": config.KLINE_INTERVAL, "CCI_PEAK": config.CCI_PEAK, "OPERATION_EXPIRY_TIME": config.OPERATION_EXPIRY_TIME, "START_GAP_PERCENTAGE": config.START_GAP_PERCENTAGE, "MAX_WEIGHT_ALLOCATION": config.MAX_WEIGHT_ALLOCATION}
                ...
                for i3, start_gap_percentage in enumerate(configs["START_GAP_PERCENTAGE"]):
                    for i4, cci_peak in enumerate(configs["CCI_PEAK"]):
                        for i5, position_structure in enumerate(configs["POSITION_STRUCTURE"]):
                            ...
                            bots.append(Bot("sandbox", this_config["PAIR"], this_config["TRADE_AMOUNT"], this_config["TAKE_PROFIT"], this_config["STOP_LOSS"],  this_config["POSITION_STRUCTURE"], this_config["KLINE_TO_USE_IN_PROD"], this_config["KLINE_INTERVAL"], this_config["CCI_PEAK"], this_config["OPERATION_EXPIRY_TIME"], this_config["START_GAP_PERCENTAGE"], max_weight_allocation=this_config["MAX_WEIGHT_ALLOCATION"]))
        return bots
    ...
    for i, bot in enumerate(bots):
        ...
        cummulative_score += result["score"]
