                ...
                for i3, start_gap_percentage in enumerate(configs["START_GAP_PERCENTAGE"]):
                    for i4, cci_peak in enumerate(configs["CCI_PEAK"]):
                        for i5, position_structure in enumerate(configs["POSITION_STRUCTURE"]):
                            ...
                            bots.append(Bot("sandbox", this_config["PAIR"], this_config["TRADE_AMOUNT"], this_config["TAKE_PROFIT"], this_config["STOP_LOSS"],  this_config["POSITION_STRUCTURE"], this_config["KLINE_TO_USE_IN_PROD"], this_config["KLINE_INTERVAL"], this_config["CCI_PEAK"], this_config["POSITION_EXPIRY_TIME"], this_config["SCORE_FILTER"], this_config["SCORE_LONGITUDE"], this_config["START_GAP_PERCENTAGE"]))
        return bots

    bots =  get_bots()
    results=[]
    period = ['1 Dic, 2021', '15 Dic, 2021']
    candles = None
    cummulative_score = 0
