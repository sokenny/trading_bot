...
config_variants = {"STOP_LOSS": [1,3,5], "TAKER_PROFIT": [1,1.5,2], "START_GAP_PERCENTAGE": [0, 0.5, 1]}
    ...
    for i1, stop_loss in enumerate(config_variants["STOP_LOSS"]):
        for i2, taker_profit in enumerate(config_variants["TAKER_PROFIT"]):
            for i3, start_gap_percentage in enumerate(config_variants["START_GAP_PERCENTAGE"]):
                ...
                this_config = get_config(default_copy, {"STOP_LOSS": stop_loss, "TAKER_PROFIT": taker_profit, "START_GAP_PERCENTAGE": start_gap_percentage})
...
bots =  get_bots()
results=[]
period = ['16 Apr, 2022', '21 Apr, 2022']
